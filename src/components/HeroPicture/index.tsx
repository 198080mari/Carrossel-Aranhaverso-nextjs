...
export default function HeroPicture({ hero }: IProps) {
  return (
    <Image
      src={heroesImage[hero.id] || ImageSpiderMan616} //se passsar id que não existe, não fica nulo
      alt={`${hero.name} (Universo-${hero.universe})` || ""}
      priority //img mais interessantes
    />
  );
}

//priority carrega imagem com prioridade
